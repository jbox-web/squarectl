  ...
  module Commands
    module Compose
      def run_docker_compose(action, args)
        run_command("docker-compose", args: docker_compose_args(action, args), env: task_env_vars)
      end

      def exec_docker_compose(action, args)
        exec_command("docker-compose", args: docker_compose_args(action, args), env: task_env_vars)
      end
